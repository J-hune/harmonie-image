...
public:
    ...
    // conversions
    Vec3 convertFrom(PixelType t) const; // from t to rgb

    Vec3 convertTo(PixelType t) const; // from rgb to t

    inline Vec3 convert(PixelType base, PixelType target) const {
        return convertFrom(base).convertTo(target);
    }
};
